...
| --- | --- | --- |
| **Header** | Supplied `mile42-logo-dark` lockup at 120x60, inline nav, CTA button, "Menu" toggle under `lg`. The lockup now sets the bar's height rather than the 52px CTA. Horizontal inset matches the page grid: padding outside `max-w-site`, as `Section` and `Footer` do, and the mobile panel steps at `md` with them. | No comp, and the PDFs define **no clearspace or minimum size** for the lockup, so the size is derived from rendering rather than from the brand. Both axes are whole pixels and the asset is an exact 2:1, because a fractional box puts the mark's strokes off the device pixel grid. The size climbed 40 to 48 to 60 as measurement showed the mark is unhinted: identical strokes land on different pixel phases and paint at different weights, 2.2px stems at 48px read as uneven, 2.8px at 60px narrows but does not close the gap. A pixel-fitted small-use master is the actual fix and is with the designer. The asset's `viewBox` is cropped to its own artwork bounds so the drawn mark keeps the #23 grid inset; the export's padded `0 0 288 144` box would have pushed it 8px inside page content. The inset was corrected in #23; before that the wordmark sat 48px inside page content above 1336px, and the open panel sat 24px outside it between 768px and 1023px. |
