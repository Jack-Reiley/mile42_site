...
The canonical workflow is shared with Codex under `.agents/`. The six Claude
slash commands in `.claude/commands/` are thin adapters and contain no separate
delivery logic:
...
Read the matching `.agents/skills/<name>/SKILL.md` and all required references.
Use `.agents/software-delivery.config.json` for project settings and
`.agents/software-delivery.workflow.json` for lifecycle states.

`design-ticket` adds stable `SCN-NNN` Given/When/Then acceptance scenarios.
`implement-ticket` creates the versioned `docs/requirements/` contract, tests,
branch, commits, and PR. Requirements map E2E coverage but never drive or
generate test execution. `verify-ticket` independently checks every scenario on
the existing PR and never creates the PR.

Failed current-ticket scenarios return to implementation on the same branch and
PR. `reconcile-ticket` may create designed follow-ups only for adjacent work
found after the original contract passes. `deliver-tickets` chooses shared,
independent, or genuinely stacked units from real relationships and affected
code, never argument order. Automation never merges.
